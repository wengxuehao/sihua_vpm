...
class DevLiveView(LiveView, View):
    ...
    def post(self, request):
        try:
            ...
            if rec_data["code"] >= 400 or rec_data["code"] < 200:
                logger.error('返回直播地址异常[message:%s][camera_id:%s]' % (rec_data['code'],camera_id))
                return JsonResponse(data=rec_data)
            else:
                logger.info('返回直播地址成功[message:%s][camera_id:%s]' % (rec_data['code'],camera_id))
                ...
                if end_time == 0:
                    try:
                        ...
                        if rec_data1['message'] == "录像信息为空" or rec_data1['data'] == {} or rec_data1['data']=='':
                            ...
                            return result.result(data=rec_data1)
